...
auto evaluate_division2(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for //"};
    }
...
//reszta z dzielenia
auto evaluate_division3(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for -"};
    }
...
//potegowanie
auto evaluate_exponentation(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for **"};
    }
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_division3(stack);
            } else if (each == "**") {
                evaluate_exponentation(stack);
            } else if (each == "sqrt") {
                evaluate_sqrt(stack);
            } else if (each == "cbrt") {
                evaluate_cbrt(stack);
